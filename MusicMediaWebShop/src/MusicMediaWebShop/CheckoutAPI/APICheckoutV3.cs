...
namespace MusicStore.Services.NganLuongAPI
{
    public class APICheckoutV3
    {

        public async System.Threading.Tasks.Task<ResponseInfo> GetUrlCheckoutAsync(RequestInfo requestContent, string payment_method = "NL")
        {

            requestContent.Payment_method = payment_method;
            String requestinfo = GetParamPost(requestContent);
            String result = await HttpPostAsync(requestinfo);
        ...
        public async System.Threading.Tasks.Task<ResponseCheckOrder> GetTransactionDetailAsync(RequestCheckOrder info)
        {
            ...
            String result = await HttpPostAsync(request);
            result = result.Replace("&", "&amp;");
            XmlDocument dom = new XmlDocument();
            dom.LoadXml(result);
            XmlNodeList root = dom.DocumentElement.ChildNodes;
        ...
        private static async System.Threading.Tasks.Task<string> HttpPostAsync(string postData)
        {
            ...
            // Prepare web request...
            var myRequest = (HttpWebRequest)WebRequest.Create("https://www.nganluong.vn/checkout.api.nganluong.post.php");
